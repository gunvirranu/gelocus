...
// static constexpr double OMEGA_EARTH = 7.292115e-5;  // [rad/s]
// static const Vec EARTH_ANG_VEL = Vec(0, 0, OMEGA_EARTH);
...
template <Frame F>
template <Frame To>
Position<To> Position<F>::transform(double jd, const EOPData eop) const {
    std::cout << "Converting from " << F <<  " to " << To << "\n";
    const auto A = Transformation<F, To>(jd, eop);
    std::cout << "Using " << A << "\n";
    return A.apply(*this);
}

// // FIXME: Determine when valid range
// double datetime_to_jd(int Y, int M, int D, int h, int m, double s) {
//     return 1721013.5 + 367.0*Y - std::floor(7.0/4 * (Y + std::floor((M+9.0) / 12.0)))
//         + std::floor(275.0/9 * M) + D + (60*h + m + s/60) / 1440;
// }
...
// FIXME: Find proper `fmod`
static constexpr double greenwich_mean_sidereal_time(const double jc_ut1) {
    constexpr double cs[] = {
...
// For a translation-unit local type
namespace {
...
static void teme_to_pef(const double jd, Matrix &S) {
    const double jc = jd_to_jc(jd);
    const double gmst = greenwich_mean_sidereal_time(jc);
...
// Implemented simplest transformations
...
Transformation<Frame::TOD, Frame::MOD>::Transformation(const double jd, const EOPData eop) {
    ...
    iau80_nutation(jc, eop, this->mat, mean_eps, omega, delta_psi);
}
